...
import { AnimeBasic, Ban, ClubBasic, MangaBasic, Message, MessageType, User, UserBasic, UserFavourites, UserHistoryRecord, UserId, UserRateExtended, UserRateStatus, UserUnreadMessages } from '../types';
...
export const users = ({ get }: RequestMethods) => {
   ...
   * @param params
   */
  const info = ({ id }: Id<UserId>): Promise<User> => get(`/users/${id}/info`, {});
   ...
   * @param params
   */
  const bans = ({ id }: Id<UserId>): Promise<Ban[]> => get(`/users/${id}/bans`, {});
