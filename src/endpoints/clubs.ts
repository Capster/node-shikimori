...
export const clubs = ({ get, post, patch }: RequestMethods) => {
   ...
   * @param params
   */
  const byId = ({ id }: Id<ClubId>): Promise<Club> => (
    get(`/clubs/${id}`, {})
  );
   ...
   * @param params
   */
  const animes = ({ id, ...params }: ClubsContentParams): Promise<AnimeBasic[]> => (
   ...
   * @param params
   */
  const mangas = ({ id, ...params }: ClubsContentParams): Promise<MangaBasic[]> => (
   ...
   * @param params
   */
  const ranobe = ({ id, ...params }: ClubsContentParams): Promise<RanobeBasic[]> => (
   ...
   * @param params
   */
  const characters = ({ id, ...params }: ClubsContentParams): Promise<CharacterBasic[]> => (
   ...
   * @param params
   */
  const collections = ({ id, ...params }: ClubsContentParams): Promise<Topic[]> => (
   ...
   * @param params
   */
  const members = ({ id, ...params }: ClubsContentParams): Promise<UserBasic[]> => (
